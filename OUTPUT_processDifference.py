...
def outputProcessDiff(projName):
	...
	wb.save(filename='./Results/' + projName + '/' + date + '-ProcessDifference.xlsx')

	wb = openpyxl.load_workbook(filename='./Results/' + projName + '/' + date + '-ProcessDifference.xlsx')
	writer = pd.ExcelWriter('./Results/' + projName + '/' + date + '-ProcessDifference.xlsx', engine='openpyxl')
	...
	for index, row in images.iterrows():
		...
		merged = netstat_process.merge(mem_process, how='outer').merge(wmi_process, on='pid', how='outer')
