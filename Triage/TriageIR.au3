...
Func TriageGUI()						;Creates a graphical user interface for Triage
   ...
   Global 	$hostn_chk, $AutoRun_chk, $AutoRun_Target_chk, $AcctInfo_chk, $IPs_chk, $CONN_chk
   Global 	$Routes_chk, $ARP_chk, $DNS_chk, $NBT_chk, $nShare_chk, $fw_chk, $host_chk, $exifmetadata_chk
   ...
   GUICreate("Triage:  KPMG Incident Response", 810, 300)
	  ...
	  GUICtrlCreateTabItem("System Information")
		 ...
		 $hostn_chk = GUICtrlCreateCheckbox("Hostname Information", 10, 150)
			GUICtrlSetTip($hostn_chk, "Determine the system's hostname.")
	  ...
	  GUICtrlCreateTabItem("Network Information")
		 ...
		 $fw_chk = GUICtrlCreateCheckbox("Firewall Information", 400, 30)
			GUICtrlSetTip($fw_chk, "Gather Firewall Settings.")
		 $host_chk = GUICtrlCreateCheckbox("Hosts Information", 400, 50)
			GUICtrlSetTip($host_chk, "Collect host file containing mapping of IP address to host names.")
	  ...
	  GUICtrlCreateTabItem("Disk Information")
		 ...
		 $exifmetadata_chk = GUICtrlCreateCheckbox("Metadata Information", 10, 110)
			GUICtrlSetTip($VolInfo_chk, "Collect Metadata Information using Exiftool.")
	  ...
	  GUICtrlCreateTabItem("Evidence Collection")
		...
		$AmCache_chk = GUICtrlCreateCheckbox("Collect AmCache.hve (Win7 >)", 400, 30)
			GUICtrlSetTip($AmCache_chk, "Collect a copy of AmCache.hve and RecentFileCache.bcf for Win7 and above.")
	  ...
	  While 1
		 ...
		 If $msg = $run Then
			...
			If (GUICtrlRead($AmCache_chk) = 1) Then
			   AmCache()
			   $fcnt = $fcnt + 1
			   GUICtrlSetData($progress, (($fcnt/$p_chkc)*100))
			EndIf
			...
			If (GUICtrlRead($hostn_chk) = 1) Then
			   Hostname()
			   $fcnt = $fcnt + 1
			   GUICtrlSetData($progress, (($fcnt/$p_chkc)*100))
			EndIf
			...
			If (GUICtrlRead($exifmetadata_chk) = 1) Then
			   exifmetadata()
			   $fcnt = $fcnt + 1
			   GUICtrlSetData($progress, (($fcnt/$p_chkc)*100))
			EndIf

			If (GUICtrlRead($fw_chk) = 1) Then
			   Firewall()
			   $fcnt = $fcnt + 1
			   GUICtrlSetData($progress, (($fcnt/$p_chkc)*100))
			EndIf
...
Func INI_Check($ini_file)				;Check the INI file included in triage for functions and whether or not to run them
   ...
   $exifmetadata_ini = IniRead($ini_file, "Function", "ExifMetadata", "Yes")
   ...
   $AmCache_ini = IniRead($ini_file, "Function", "AmCache", "Yes")
...
Func _Ini2GUI()							;Correlate the INI into checking the boxes of the GUI to execute the specific functions
   ...
   If $hostn_ini = "Yes" Then
	  GUICtrlSetState($hostn_chk, $GUI_CHECKED)
   Else
	  GUICtrlSetState($hostn_chk, $GUI_UNCHECKED)
   EndIf

   If $host_ini = "Yes" Then
   ...
   If $fw_ini = "Yes" Then
	  GUICtrlSetState($fw_chk, $GUI_CHECKED)
   Else
	  GUICtrlSetState($fw_chk, $GUI_UNCHECKED)
   EndIf
   ...
   If $exifmetadata_ini = "Yes" Then
	  GUICtrlSetState($exifmetadata_chk, $GUI_CHECKED)
   Else
	  GUICtrlSetState($exifmetadata_chk, $GUI_UNCHECKED)
   EndIf

   If $AmCache_ini = "Yes" Then
	  GUICtrlSetState($AmCache_chk, $GUI_CHECKED)
   Else
	  GUICtrlSetState($AmCache_chk, $GUI_UNCHECKED)
   EndIf
...
Func ProgChkCount()						;Count number of functions executing for GUI Progress Bar
   ...
   If (GUICtrlRead($AmCache_chk) = 1) Then $p_chkc = $p_chkc + 1
   ...
   If (GUICtrlRead($hostn_chk) = 1) Then $p_chkc = $p_chkc + 1
   If (GUICtrlRead($host_chk) = 1) Then $p_chkc = $p_chkc + 1
   If (GUICtrlRead($fw_chk) = 1) Then $p_chkc = $p_chkc + 1
   ...
   If (GUICtrlRead($exifmetadata_chk) = 1) Then $p_chkc = $p_chkc + 1
EndFunc
