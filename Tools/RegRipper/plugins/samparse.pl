...
sub pluginmain {
    ...
    ::rptMsg('//If Microsoft Live ID is used for logon, "Logon Count" is 0. Check for "SAM\Domains\Account\Users\InternetUserName" Key');
	::rptMsg('//This is important for Office365 accounts!'."\n");
	my $reg = Parse::Win32Registry->new($hive);
	my $root_key = $reg->get_root_key;
	::rptMsg("");
# Get user information
	::rptMsg("Profiling Users");
	::rptMsg("-" x 25);
...
# Get Group membership information
	...
	::rptMsg("Source: ".$key_path);
	::rptMsg('//If Last Write to group "Administrators" is same as Account Created time for User, it means there is only ONE administrator!');
	::rptMsg("-" x 25);
	if ($key = $root_key->get_subkey($key_path)) {
		my %grps;
