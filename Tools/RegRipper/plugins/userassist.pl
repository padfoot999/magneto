...
my %folder_guid = ("{de61d971-5ebc-4f02-a3a9-6c82895e5c04}" => "Add or Remove Programs",
...
sub pluginmain {
	...
	my $key;
	$workbook_name = $ntuser;
	$workbook_name =~ s/(.*\\)([^\\]*)_(USER_[^\\]*).dat$/$1Timeline-UserAssist-$3.xlsx/g;
	$reg_key = $ntuser;
...
sub processKey {
	...
	$workbook = Excel::Writer::XLSX->new($workbook_name);
...
#				}
				...
				my $runtime = gmtime($time_val);
				my $runtime_parsed;
				if ($runtime !~ m/(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) {2,}/) {
