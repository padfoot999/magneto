...
sub pluginmain {
    ...
    ::rptMsg('//Shellbag information is available for folders that have been opened and closed in Windows Explorer at least once.');
    ::rptMsg('//Can track user window viewing preferences to Windows Explorer');
    ::rptMsg('//Can be utilized to tell if activity occurred in a folder');
    ::rptMsg('//You can track if a user opened/closed/created/deleted/copied a folder');
    ::rptMsg('//In some cases, you can see the files from a specific folder as well');
    ...
    my %paths = ("Win81" => "Software\\Microsoft\\Windows\\Shell\\BagMRU",
                 "Win7" => "Local Settings\\Software\\Microsoft\\Windows\\Shell\\BagMRU",
