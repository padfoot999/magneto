...
def malwrUploadDaemon(daemonname, inqueue, outqueue, queues):
    api_unauthenticated = MalwrAPI(username=CONFIG['VTCHECKER']['MALWR_LOGIN_USERNAME'], password=CONFIG['VTCHECKER']['MALWR_LOGIN_PASSWORD'], verbose=True)
    while True:
        try:
            ...
            if results == None:
                ...
                results = api_unauthenticated.submit_sample(item)
...
def malwrUploadMaster(hfp=HashFilePath()):
    ...
    if daemoncount > 0:
        for i in xrange(daemoncount):
            ...
            thread = Thread(target=malwrUploadDaemon, args=(i, queues['uploadqueue'], queues['resultsqueue'], queues))
